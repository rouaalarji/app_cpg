...
async function create({ code, nom, departementId, chefId, description }) {
  const [result] = await db.query(
    'INSERT INTO service (code, nom, departement_id, chef_id, description) VALUES (?, ?, ?, ?, ?)',
    [code, nom, departementId, chefId || null, description || null]
  );
  return result.insertId;
}

async function update(id, { code, nom, departementId, chefId, description }) {
  await db.query(
    'UPDATE service SET code = ?, nom = ?, departement_id = ?, chef_id = ?, description = ? WHERE id = ?',
    [code, nom, departementId, chefId || null, description || null, id]
  );
}
