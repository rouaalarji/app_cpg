...
{ to: '/admin/types-conge', label: 'Types de congés', icone: 'bi-list-check' },    { to: '/admin/absences', label: 'Absences', icone: 'bi-calendar-x' },
  ...
  const liensAdmin = [
    { to: '/admin/comptes', label: 'Gestion des comptes', icone: 'bi-shield-lock' },
  ];

  const liens = role === 'ADMIN' ? [...liensCommuns, ...liensAdmin] : liensCommuns;
      ...
      {ouvert && (
        <div className="px-3 mb-2">
          <span className="badge bg-light text-dark">
            {role === 'ADMIN' ? 'Administrateur' : 'Ressources Humaines'}
          </span>
        </div>
      )}
